...
for (i in year:year){
  for (j in month:month){
    ...
    if (exists("df2")){
      ...
      df_itogo2 <- complete(sqldf(query_compos('\"ITOGO\"', 'df2')))
    
      df_itogo <- rbind(df_itogo, df_itogo2) %>%distinct()
      remove(df2)
    }
    ...
    if (j == 4){
      ...
      df_rub_7 <- sqldf(subtract('df_rub', 'df_rub_q'))
      df_inval_7 <- sqldf(subtract('df_inval', 'df_inval_q'))
      df_itogo_7 <- sqldf(subtract('df_itogo', 'df_itogo_q'))
      ...
      # rewrite data as a previous for next quarter
      df_rub_q <- df_rub
      df_inval_q <- df_inval
      df_itogo_q <- df_itogo
    }
